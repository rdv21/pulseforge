...
HOP_SIZE = 1024      # Update spectrum every ~21ms (~48fps)
...
def _compute_spectrum(samples: np.ndarray, fft_size: int, num_points: int) -> list:
    ...
    # Resample to log-spaced points (20Hz..20kHz)
    log_min = np.log(20)
    log_max = np.log(20000)
    points = []
    for i in range(num_points):
        t = i / (num_points - 1)
        target_freq = np.exp(log_min + t * (log_max - log_min))
        idx = np.argmin(np.abs(bin_freqs - target_freq))
        lo = max(0, idx - 1)
        ...
        points.append({"freq": float(target_freq), "level": level})
...
class NativeMicChain:
    ...
    def __init__(self):
        ...
        self.noise = dsp.SpeexNoiseProcessor()
    ...
    def _process_loop(self):
        ...
        while self._running:
            try:
                ...
                if raw_peak < 1e-7:
                    _silence_count += 1
                    if _silence_count >= 50 and not _kicked:  # ~5 seconds of silence
                        print("  NativeMicChain: source appears suspended, redirecting...")
                        self._redirect_capture()
                        _kicked = True
                else:
                    _silence_count = 0
                ...
                # Process: noise suppression → gate → EQ → compressor
                # Speex works on the raw signal directly — no pre-gain needed
                # (unlike RNNoise which needed +24dB to detect speech)
                ...
                if self._spec_hop_counter >= HOP_SIZE:
                    self._spec_hop_counter = 0
                    buf = np.roll(self._spec_buffer, -self._spec_write_pos)
                    spectrum = _compute_spectrum(buf, FFT_SIZE, NUM_SPEC_POINTS)
                    with self._spec_lock:
                        self._spectrum = spectrum

                # Write to playback (stdin)
                try:
                    if self._playback_proc.poll() is not None:
                        ...
                        if not self._restart_playback():
                            break
                    self._playback_proc.stdin.write(block.tobytes())
                    self._playback_proc.stdin.flush()
